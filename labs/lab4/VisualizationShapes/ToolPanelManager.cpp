...
ToolPanelManager::ToolPanelManager() 
{
    ...
    m_lastPosition = std::stack<sf::Vector2i>();
}
...
void ToolPanelManager::SetLastPosition(const sf::Vector2i& position)
{
    m_lastPosition.push(position);
}

sf::Vector2i ToolPanelManager::GetLastPosition()
{
    sf::Vector2i position = m_lastPosition.top();
    m_lastPosition.pop();
    return position;
}
