...
void Composite::Draw(sf::RenderWindow& window) const 
{
    ...
    {
        if (child->GetType() == RECTANGLE)
        {
            CRectangleDrawDecorator rectangle(child.get());
            rectangle.Draw(window);
        }
        else if (child->GetType() == CIRCLE)
        {
            CCircleDrawDecorator circle(child.get());
            circle.Draw(window);
        }
        else if (child->GetType() == TRIANGLE)
        {
            CTriangleDrawDecorator triangle(child.get());
            triangle.Draw(window);
        }
        else if (child->GetType() == COMPOSITE)
        {
            child->Draw(window);
        }
    }
...
void Composite::Add(std::shared_ptr<CShape> component)
{
    if (m_children.empty())
    {
...
void Composite::Remove(std::shared_ptr<CShape> component) 
{
    m_children.erase(std::remove(m_children.begin(), m_children.end(), component), m_children.end());
}

std::vector<std::shared_ptr<CShape>> Composite::GetChildren() const 
{
    return m_children;
}
